/**
 * The `gpio` module supports GPIO pin abstraction. Use `require('gpio')` to
 * access this module.
 */
declare module 'gpio' {
  ...
  class GPIO {
     ...
     * - **`mode`** `<number>` The pin mode `INPUT (0)` , `OUTPUT (1)` ,
     *   `INPUT_PULLUP (2)` or `INPUT_PULLDOWN (3)` . **Default:** `INPUT`
     *
     ...
     * ```
     */
    constructor(pin: number, mode?: PinMode.All)
     ...
     * - **`event`** `<number>` set the event of the `pin`. There are three
     *   events,`FALLING (4)`, `RISING (8)`, and `CHANGE (12)`. **Default:** `CHANGE`.
     *
     * Add interrupt on the GPIO pin. Run the **`callback`** function when the
     * **`event`** is triggered on the **`pin`**. There are three events for the
     * interrupts. The `FALLING (4)` event is triggered when the **`pin`** state
     * is changed from `HIGH` to `LOW`. The `RISING (8)` event is triggered when
     * the **`pin`** state is changed from `LOW` to `HIGH`. The `CHANGE (12)`
     * event is triggered when the **`pin`** state is changed to any states,
     * which means the `CHANGE` event is the same as the `FALLING` + `RISING` events.
     *
     ...
     */
    irq(
      callback: (pin: number, event: SignalEvent.All) => void,
     ...
     * - `<number>`
     *
     * Current mode of the GPIO pin. The value is `INPUT (0)` , `OUTPUT (1)` ,
